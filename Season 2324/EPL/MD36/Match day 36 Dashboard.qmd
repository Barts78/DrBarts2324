...
Vizlocation <- paste0("~/Dropbox/AI lfc data/UP Pod Prep 2324/Matchday 36/")

#set dashboard directory
Dashlocation <- paste0("~/Dropbox/AI lfc data/UP Pod Prep 2324/Matchday 36/Viz Pack/")

#set oppo
Match_Oppo <- paste("Tottenham Hotspur")

#set oppo color
Color_Oppo <- paste("#132257")
...
ColorScale_Away <- paste("ggsci::indigo_material") 
...
MatchIdUS <- 22251 #check

#Set Fotmob Match ID
MatchIdFotmob <- 4193887 #check
...
MatchIdFbref <- paste("https://fbref.com/en/matches/08341569/Liverpool-Tottenham-Hotspur-May-5-2024-Premier-League") #check

#set Game name
GameName <- paste("LFC_vs_THFC")
...
GameNamePlayers <- paste("LFC_vs_THFC_players")
...
LFC_Understat_shots_EPL2324 <- LFC_Understat_shots_EPL2324 %>%
  mutate(away_team = case_when(away_team == "Brighton" ~ "Brighton & Hove Albion",
    ...
    mutate(home_team = case_when(home_team == "West Ham" ~ "West Ham United",
...
Oppo_shotsUS <- LFC_Understat_shots_EPL2324 %>%
    mutate(away_team = case_when(away_team == "Luton" ~ "Luton Town", TRUE ~ as.character(away_team))) %>%
      mutate(away_team = case_when(away_team == "Tottenham" ~ "Tottenham Hotspur", TRUE ~ as.character(away_team))) 
...
lineupdataAway <- lineupdata %>%
  ...
  cols_width(col = c(5:6) ~ px(50)) %>%
  tab_style(
    style = list(cell_text(weight = "bold", color = Color_Oppo_Text, align = "left"), 
                 cell_fill(color = Color_Oppo)),
    locations = cells_row_groups(groups = everything())) %>%
  tab_style(
    locations = cells_title(groups = "title"),
    style = list(cell_text(weight = "bold", size = 16))) %>%
...
MCFCSeason_MC_Plot <- ggplot(MCFCpoint_probs, aes(x = total_points, y = prob)) +
  ...
  labs(title = "Manchester City Final Predicted Point Totals (1000 Sims)",
       ...
       y = "Probability %",
       caption = my_socials) +
  scale_x_continuous(limits = c((min(MCFCseason_totals$total_points)), (sum(MCFCcurrent_points + remaining_games + 1 * 3))), breaks = seq(0, (sum(MCFCcurrent_points + remaining_games + 1 * 3)), 1)) +
  scale_y_continuous(limits = c(0, 50), breaks = seq(0, 50, 5)) +
  theme(plot.background = element_rect(colour = "snow", fill = "snow"),
        plot.caption = element_markdown(),
        panel.background = element_rect(colour = "snow", fill = "snow"),
...
LFCpassPlotPost1stsubsWD <- plot_passnet_AI(data = plottingData, 
                                        team_name = Match_LFC, scale_stat = "EPV", 
                                        scale_color = Color_LFC,  text_color = Color_LFC_Text, subtitle = paste0(Match_Fixture, " Post 1st Subs"),
                                        plot_start = firstSub, plot_finish = 100, pitch = "", wrap = "", pressmap = "Yes", logo = "", passes = 4)
LFCpassPlotPost1stsubsWD
```
...
OppopassPlotpost1stsubsWD <- plot_passnet_AI(data = plottingData, 
                                        team_name = Match_Oppo, scale_stat = "EPV", 
                                        scale_color = Color_Oppo,  text_color = Color_Oppo_Text, subtitle = paste0(Match_Fixture, " Post 1st Subs"),
                                        plot_start = firstSub, plot_finish = 100, pitch = "", wrap = "", pressmap = "Yes", logo = "", passes = 4)
OppopassPlotpost1stsubsWD
```
...
Zone14_HS_Passes_Oppo_Plot <- Z14_HS_Passes_Oppo_Data %>%
  ...
  facet_grid(factor(Pass_Area, levels = c("LHS", "Zone 14", "RHS")) ~ Game_Period) +
  labs(title = paste0("Zone 14 and HS ", OppoPPsTitle_text), 
       subtitle = paste0(Match_Fixture),
       caption = My_caption) +
  theme(plot.title = element_markdown(),
        plot.subtitle = element_text(colour = "black", size = 12),
