...
Vizlocation <- paste0("~/Dropbox/AI lfc data/UP Pod Prep 2324/Matchday 31/")

#set dashboard directory
Dashlocation <- paste0("~/Dropbox/AI lfc data/UP Pod Prep 2324/Matchday 31/Viz Pack/")

#set oppo
Match_Oppo <- paste("Manchester United")

#set oppo color
Color_Oppo <- paste("#C70101")

#set oppo text
Color_Oppo_Text <- paste("#030303")

#Set LFC
Match_LFC <- paste("Liverpool")

#set Home color
Color_LFC <- paste("#1B5E20")
...
MatchIdUS <- 22211 #check

#Set Fotmob Match ID
MatchIdFotmob <- 4193847 #check
...
MatchIdFbref <- paste("https://fbref.com/en/matches/d57ffe1a/North-West-Derby-Manchester-United-Liverpool-April-7-2024-Premier-League") #check

#set Game name
GameName <- paste("MUFC_vs_LFC")
...
GameNamePlayers <- paste("MUFC_vs_LFC_players")
...
PPsTitle_text <- paste0(
              glue("<span style='font-family:\"Arial Bold\";color:{Color_LFC};font-size:16px;'>{Match_LFC}</span><span style='font-family:\"Arial\";color:gray12;font-size:16px;'> Passes (</span><span style='font-family:\"Arial Bold\";color:black;font-size:16px;'>Successful</span><span style='color:gray12;font-size:16px;'> and </span><span style='font-family:\"Arial Bold\";color:{Color_LFC};font-size:16px;'>Progressive</span><span style='color:gray12;font-size:16px;'>) by Game Period,</span>"))

OppoPPsTitle_text <- paste0(
              glue("<span style='font-family:\"Arial Bold\";color:{Color_Oppo};font-size:16px;'>{Match_Oppo}</span><span style='font-family:\"Arial\";color:gray12;font-size:16px;'> Passes (</span><span style='font-family:\"Arial Bold\";color:#1E1E1E;font-size:16px;'>Successful</span><span style='color:gray12;font-size:16px;'> and </span><span style='font-family:\"Arial Bold\";color:{Color_Oppo};font-size:16px;'>Progressive</span><span style='color:gray12;font-size:16px;'>) by Game Period,</span>"))
...
fileName2 <- paste0("~/Dropbox/AI lfc data/data sets/understat/EPL_Shots_2324.xlsx")
Understat_shots_EPL2324 <- read_xlsx(fileName2)
...
#shot map LFC data
LFC_Understat_shots_EPL2324 <- LFC_Understat_shots_EPL2324 %>%
  mutate(away_team = case_when(away_team == "Brighton" ~ "Brighton & Hove Albion",
...
#load data
LFC2324ShotDatabase <- Understat_shots_EPL2324

#manipluate data
ShotsA <- LFC2324ShotDatabase %>%
  filter(h_a == "a")

ShotsH <- LFC2324ShotDatabase %>%
  filter(h_a == "h")
...
#### man city
MC2324ShotDatabase <- Understat_shots_EPL2324

#manipluate data
ShotsA <- MC2324ShotDatabase %>%
  filter(h_a == "a")

ShotsH <- MC2324ShotDatabase %>%
  filter(h_a == "h")
...
MCFCShotsALL <- bind_rows(MCFCShotsA, MCFCShotsH)
...
remaining_games <- 38 - MatchDay
#- n_games
MCFCcurrent_points <- MC_League_Points
win_prob <- 0.5   # Probability of winning a game (adjust as needed)
draw_prob <- 0.5 # Probability of drawing a game (adjust as needed)
#loss_prob <- 1 - win_prob - draw_prob
loss_prob <- 0.5
...
# Simulate goals scored and conceded using appropriate distributions
# (adjust distributions and parameters based on your data or assumptions)
goals_scored <- rpois(n_sims * n_games, lambda = MCFCShotsALL$xG)  # Poisson distribution for goals scored
goals_conceded <- rpois(n_sims * n_games, lambda = OppoShotsALL$xGA)  # Poisson distribution for goals conceded
...
# Calculate total points per season for each simulation
MCFCseason_totals <- sim_data %>%
  group_by(sim_id) %>%
  summarise(total_points = sum(points) * P_Season + MCFCcurrent_points)

# Limit total points to the maximum possible (current + remaining games * 3)
MCFCseason_totals <- MCFCseason_totals %>%
  mutate(total_points = pmin(total_points, MCFCcurrent_points + remaining_games * 3))

# Calculate and summarize final point probabilities
MCFCpoint_probs <- MCFCseason_totals %>%
  mutate(total_points = round(total_points)) %>%  
  count(total_points) %>%
  mutate(prob = (n / n_sims) * 100) %>%
  arrange(total_points)
...
AFC2324ShotDatabase <- Understat_shots_EPL2324

#manipluate data
ShotsA <- AFC2324ShotDatabase %>%
  filter(h_a == "a")

ShotsH <- AFC2324ShotDatabase %>%
  filter(h_a == "h")
...
AFCShotsALL <- bind_rows(AFCShotsA, AFCShotsH)
...
remaining_games <- 38 - MatchDay
#- n_games
AFCcurrent_points <- AFC_League_Points
win_prob <- 0.5   # Probability of winning a game (adjust as needed)
draw_prob <- 0.5 # Probability of drawing a game (adjust as needed)
#loss_prob <- 1 - win_prob - draw_prob
loss_prob <- 0.5
...
# Simulate goals scored and conceded using appropriate distributions
# (adjust distributions and parameters based on your data or assumptions)
goals_scored <- rpois(n_sims * n_games, lambda = AFCShotsALL$xG)  # Poisson distribution for goals scored
goals_conceded <- rpois(n_sims * n_games, lambda = OppoShotsALL$xGA)  # Poisson distribution for goals conceded
...
# Calculate total points per season for each simulation
AFCseason_totals <- sim_data %>%
  group_by(sim_id) %>%
  summarise(total_points = sum(points) * P_Season + AFCcurrent_points)

# Limit total points to the maximum possible (current + remaining games * 3)
AFCseason_totals <- AFCseason_totals %>%
  mutate(total_points = pmin(total_points, AFCcurrent_points + remaining_games * 3))

# Calculate and summarize final point probabilities
AFCpoint_probs <- AFCseason_totals %>%
  mutate(total_points = round(total_points)) %>%  
  count(total_points) %>%
  mutate(prob = (n / n_sims) * 100) %>%
  arrange(total_points)
...
MCFCSeason_MC_Plot <- ggplot(MCFCpoint_probs, aes(x = total_points, y = prob)) +
  geom_bar(stat = "identity", fill = "#69A8D8", color = "black") +
  labs(title = "Manchester City Final Predicted Point Totals (1000 Sims)",
       subtitle = paste("After Match Day ", MatchDay),
       x = "Final Points",
       y = "Probability %",
       caption = my_socials) +
  scale_x_continuous(limits = c((min(MCFCseason_totals$total_points)), (sum(MCFCcurrent_points + remaining_games * 3))), breaks = seq(0, (sum(MCFCcurrent_points + remaining_games * 3)), 1)) +
  scale_y_continuous(limits = c(0, 50), breaks = seq(0, 50, 5)) +
  theme(plot.background = element_rect(colour = "snow", fill = "snow"),
        plot.caption = element_markdown(),
        panel.background = element_rect(colour = "snow", fill = "snow"),
...
AFCSeason_MC_Plot <- ggplot(AFCpoint_probs, aes(x = total_points, y = prob)) +
  geom_bar(stat = "identity", fill = "yellow2", color = "black") +
  labs(title = "Arsenal Final Predicted Point Totals (1000 Sims)",
       subtitle = paste("After Match Day ", MatchDay),
       x = "Final Points",
       y = "Probability %",
       caption = my_socials) +
  scale_x_continuous(limits = c((min(AFCseason_totals$total_points)), (sum(AFCcurrent_points + remaining_games * 3))), breaks = seq(0, (sum(AFCcurrent_points + remaining_games * 3)), 1)) +
  scale_y_continuous(limits = c(0, 50), breaks = seq(0, 50, 5)) +
  theme(plot.background = element_rect(colour = "snow", fill = "snow"),
        plot.caption = element_markdown(),
        panel.background = element_rect(colour = "snow", fill = "snow"),
...
passing_sequencesLFC <- successful_passes %>%
  mutate(
    pass_count = ifelse(Successful_Passes == 1 & teamId == Match_LFC, 1, 0),
    passing_seq = accumulate(Successful_Passes == 1 & teamId == Match_LFC, ~ifelse(.y == 0, .y, .x + .y)),
...
passing_sequencesOppo <- successful_passes %>%
  mutate(
    pass_count = ifelse(Successful_Passes == 1 & teamId == Match_Oppo, 1, 0),
    passing_seq = accumulate(Successful_Passes == 1 & teamId == Match_Oppo, ~ifelse(.y == 0, .y, .x + .y)),
    Pchain = cumsum(passing_seq == 0) + 1,
    pass_count = cumsum(pass_count)
  ) %>%
  group_by(Pchain) %>%
...
passing_sequencesLFC_Plot <- passing_sequencesLFC %>%
  ...
  labs(title = paste0(Match_LFC, " Passing Chain Length (5, 6-10, 11-15, 16 +) by Game Period"), 
       subtitle = paste0(Match_Fixture),
       caption = My_caption) +
...
passing_sequencesOppo_Plot <- passing_sequencesOppo %>%
  ...
  facet_grid(factor(Pchain_Length, levels = c("VLow", "Low", "Medium", "High")) ~ Game_Period) +
  labs(title = paste0(Match_Oppo, " Passing Chain Length (5, 6-10, 11-15, 16 +) by Game Period"), 
       subtitle = paste0(Match_Fixture),
       caption = My_caption) +
  theme(plot.title = element_text(colour = "black", face = "bold", size = 14),
        plot.subtitle = element_text(colour = "black", size = 12),
...
Home_Passing_Chains_Table <- passing_sequencesLFC_Table %>%
  ...
  cols_align(align = "right", columns = everything()) %>%
  tab_header(
    title = paste0(Match_LFC, " FT Passing Chain Length (5, 6-10, 11-15, 16 +) & Distance (m)"),
    subtitle = paste0(Match_Fixture)) %>%
  tab_source_note(md(My_caption)) %>%
  cols_label_with(fn = ~ janitor::make_clean_names(., case = "title")) %>%
...
Home_Passing_Chains_1st_Table <- passing_sequencesLFC_1st_Table %>%
  gt() %>%
  fmt_percent(columns = c(5,9), decimals = 0) %>%
  fmt_number(columns = c(2:4), decimals = 0) %>%
  fmt_number(columns = c(6:8), decimals = 2) %>%
  cols_align(align = "right", columns = everything()) %>%
  tab_header(
    title = paste0(Match_LFC, " 1st Half Passing Chain Length (5, 6-10, 11-15, 16 +) & Distance (m)"),
    subtitle = paste0(Match_Fixture)) %>%
  tab_source_note(md(My_caption)) %>%
  cols_label_with(fn = ~ janitor::make_clean_names(., case = "title")) %>%
  grand_summary_rows(columns = c(2:4, 6:8), fns = list("Overall Totals" = ~ sum (., na.rm=TRUE)), fmt = list(~fmt_number(., decimals =0))) %>%
...
Home_Passing_Chains_2nd_Table <- passing_sequencesLFC_2nd_Table %>%
  gt() %>%
  fmt_percent(columns = c(5,9), decimals = 0) %>%
  fmt_number(columns = c(2:4), decimals = 0) %>%
  fmt_number(columns = c(6:8), decimals = 2) %>%
  cols_align(align = "right", columns = everything()) %>%
  tab_header(
    title = paste0(Match_LFC, " 2nd Half Passing Chain Length (5, 6-10, 11-15, 16 +) & Distance (m)"),
    subtitle = paste0(Match_Fixture)) %>%
  tab_source_note(md(My_caption)) %>%
  cols_label_with(fn = ~ janitor::make_clean_names(., case = "title")) %>%
  grand_summary_rows(columns = c(2:4, 6:8), fns = list("Overall Totals" = ~ sum (., na.rm=TRUE)), fmt = list(~fmt_number(., decimals =0))) %>%
...
Oppo_Passing_Chains_Table <- passing_sequencesOppo_Table %>%
  ...
  cols_align(align = "right", columns = everything()) %>%
  tab_header(
    title = paste0(Match_Oppo, " FT Passing Chain Length (5, 6-10, 11-15, 16 +) & Distance (m)"),
    subtitle = paste0(Match_Fixture)) %>%
  tab_source_note(md(My_caption)) %>%
  cols_label_with(fn = ~ janitor::make_clean_names(., case = "title")) %>%
  grand_summary_rows(columns = c(2:4, 6:8), fns = list("Overall Totals" = ~ sum (., na.rm=TRUE)), fmt = list(~fmt_number(., decimals =0))) %>%
...
Oppo_Passing_Chains_1st_Table <- passing_sequencesOppo_1st_Table %>%
  gt() %>%
  fmt_percent(columns = c(5,9), decimals = 0) %>%
  fmt_number(columns = c(2:4), decimals = 0) %>%
  fmt_number(columns = c(6:8), decimals = 2) %>%
  cols_align(align = "right", columns = everything()) %>%
  tab_header(
    title = paste0(Match_Oppo, " 1st Half Passing Chain Length (5, 6-10, 11-15, 16 +) & Distance (m)"),
    subtitle = paste0(Match_Fixture)) %>%
  tab_source_note(md(My_caption)) %>%
  cols_label_with(fn = ~ janitor::make_clean_names(., case = "title")) %>%
  grand_summary_rows(columns = c(2:4, 6:8), fns = list("Overall Totals" = ~ sum (., na.rm=TRUE)), fmt = list(~fmt_number(., decimals =0))) %>%
  ...
  tab_style(locations = cells_title(groups = "title"),
            style = list(cell_text(weight = "bold",
                                   size = 16))) %>%
  gt_color_rows(columns = c(5,9), palette = ColorScale_Away, domain = c(0,1))
Oppo_Passing_Chains_1st_Table

Oppo_Passing_Chains_2nd_Table <- passing_sequencesOppo_2nd_Table %>%
  gt() %>%
  fmt_percent(columns = c(5,9), decimals = 0) %>%
  fmt_number(columns = c(2:4), decimals = 0) %>%
  fmt_number(columns = c(6:8), decimals = 2) %>%
  cols_align(align = "right", columns = everything()) %>%
  tab_header(
    title = paste0(Match_Oppo, " 2nd Half Passing Chain Length (5, 6-10, 11-15, 16 +) & Distance (m)"),
    subtitle = paste0(Match_Fixture)) %>%
  tab_source_note(md(My_caption)) %>%
  cols_label_with(fn = ~ janitor::make_clean_names(., case = "title")) %>%
  grand_summary_rows(columns = c(2:4, 6:8), fns = list("Overall Totals" = ~ sum (., na.rm=TRUE)), fmt = list(~fmt_number(., decimals =0))) %>%
...
```{r}
RHS_Passes_LFC_Plot <- RHS_Passes_LFC_Data %>%
  ggplot(aes(x = x, y = y)) +
  annotate_pitch(dimensions = pitch_opta, fill = "#E5E5E5", colour = "#95A5A6") +
  theme_pitch() +
  geom_segment(aes(y = 21.1, yend = 21.1, x = 0, xend = 100), linewidth = 0.3, colour = "#95A5A6", linetype = "longdash") +
...
```{r}
RHS_Passes_Oppo_Plot <- RHS_Passes_Oppo_Data %>%
  ggplot(aes(x = x, y = y)) +
  annotate_pitch(dimensions = pitch_opta, fill = "#E5E5E5", colour = "#95A5A6") +
  theme_pitch() +
  geom_segment(aes(y = 21.1, yend = 21.1, x = 0, xend = 100), linewidth = 0.3, colour = "#95A5A6", linetype = "longdash") +
  ...
  scale_colour_gradientn(
    colors = c("#1E1E1E", Color_Oppo),
    values = scales::rescale(c(0, 1)), 
    breaks = scales::breaks_width(0.01), 
    name = "Progressive Pass") +
  
  facet_wrap(~ Game_Period, ncol = 4) +
  labs(title = paste0("Right HS ", OppoPPsTitle_text), 
       subtitle = paste0(Match_Fixture),
       caption = My_caption) +
  theme(plot.title = element_markdown(),
        plot.subtitle = element_text(colour = "black", size = 12),
...
RHS_Passes_Oppo_Plot
```

### Row

```{r}
LHS_Passes_LFC_Plot <- LHS_Passes_LFC_Data %>%
  ggplot(aes(x = x, y = y)) +
  annotate_pitch(dimensions = pitch_opta, fill = "#E5E5E5", colour = "#95A5A6") +
  theme_pitch() +
  geom_segment(aes(y = 21.1, yend = 21.1, x = 0, xend = 100), linewidth = 0.3, colour = "#95A5A6", linetype = "longdash") +
...
```{r}
LHS_Passes_Oppo_Plot <- LHS_Passes_Oppo_Data %>%
  ggplot(aes(x = x, y = y)) +
  annotate_pitch(dimensions = pitch_opta, fill = "#E5E5E5", colour = "#95A5A6") +
  theme_pitch() +
  geom_segment(aes(y = 21.1, yend = 21.1, x = 0, xend = 100), linewidth = 0.3, colour = "#95A5A6", linetype = "longdash") +
  ...
  facet_wrap(~ Game_Period, ncol = 4) +
  labs(title = paste0("Left HS ", OppoPPsTitle_text), 
       subtitle = paste0(Match_Fixture),
       caption = My_caption) +
  theme(plot.title = element_markdown(),
        plot.subtitle = element_text(colour = "black", size = 12),
...
LHS_Passes_Oppo_Plot
```
### Row
```{r}
Zone14_Passes_LFC_Plot <- Zone14_Passes_LFC_Data %>%
  ggplot(aes(x = x, y = y)) +
  annotate_pitch(dimensions = pitch_opta, fill = "#E5E5E5", colour = "#95A5A6") +
  theme_pitch() +
  geom_segment(aes(y = 21.1, yend = 21.1, x = 0, xend = 100), linewidth = 0.3, colour = "#95A5A6", linetype = "longdash") +
...
Zone14_Passes_LFC_Plot
```



```{r}
Zone14_Passes_Oppo_Plot <- Zone14_Passes_Oppo_Data %>%
  ggplot(aes(x = x, y = y)) +
  annotate_pitch(dimensions = pitch_opta, fill = "#E5E5E5", colour = "#95A5A6") +
  theme_pitch() +
  geom_segment(aes(y = 21.1, yend = 21.1, x = 0, xend = 100), linewidth = 0.3, colour = "#95A5A6", linetype = "longdash") +
  ...
  facet_wrap(~ Game_Period, ncol = 4) +
  labs(title = paste0("Zone 14 ", OppoPPsTitle_text), 
       subtitle = paste0(Match_Fixture),
       caption = My_caption) +
  theme(plot.title = element_markdown(),
        plot.subtitle = element_text(colour = "black", size = 12),
