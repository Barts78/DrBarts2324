...
Vizlocation <- paste0("~/Dropbox/AI lfc data/UP Pod Prep 2324/Matchday 28/")

#set dashboard directory
Dashlocation <- paste0("~/Dropbox/AI lfc data/UP Pod Prep 2324/Matchday 28/Viz Pack/")

#set Away
Match_Oppo <- paste("Manchester City")

#set Away color
Color_Oppo <- paste("#69A8D8")
...
MatchIdUS <- 22171 #check

#Set Fotmob Match ID
MatchIdFotmob <- 4193807 #check
...
MatchIdFbref <- paste("https://fbref.com/en/matches/96da3e29/Liverpool-Manchester-City-March-10-2024-Premier-League") #check

#set Game name
GameName <- paste("LFC_vs_MCFC")
...
GameNamePlayers <- paste("LFC_vs_MCFC_players")
...
# Match Summary
``` {r summary table setup, echo:FALSE, messages:FALSE, warnings:FALSE}
#|height: 50%
...
Season_MC_Plot <- ggplot(point_probs, aes(x = total_points, y = prob)) +
  ...
  labs(title = paste(Match_LFC, " Final Point Totals Post MD Result (1000 Sims)"),
       subtitle = Match_Fixture,
       x = "Final Points",
       y = "Probability %",
       caption = my_socials) +
...
Season_MC_Plot
ggsave(path = Vizlocation, "LFC Season points predictions.png", width = 2500, height = 2150, units = "px", dpi = 400)
```
...
passing_sequencesLFC <- successful_passes %>%
  mutate(
    pass_count = ifelse(`type/value` == 1 & `outcomeType/value` == 1 & Attempted_Passes == 1 & teamId == Match_LFC, 1, 0),
    passing_seq = accumulate(`type/value` == 1  & `outcomeType/value` == 1 & Attempted_Passes == 1 & teamId == Match_LFC, ~ifelse(.y == 0, .y, .x + .y)),
...
passing_sequencesOppo <- successful_passes %>%
  mutate(
    pass_count = ifelse(`type/value` == 1 & `outcomeType/value` == 1 & Attempted_Passes == 1 & teamId == Match_Oppo, 1, 0),
    passing_seq = accumulate(`type/value` == 1  & `outcomeType/value` == 1 & Attempted_Passes == 1 & teamId == Match_Oppo, ~ifelse(.y == 0, .y, .x + .y)),
    Pchain = cumsum(passing_seq == 0) + 1,
    pass_count = cumsum(pass_count)
  ) %>%
  group_by(Pchain) %>%
...
Home_Passing_Chains_Table <- passing_sequencesLFC_Table %>%
  ...
  cols_align(align = "right", columns = everything()) %>%
  tab_header(
    title = paste0(Match_LFC, " Passing Chain Length (5, 6-10, 11-15, 16 +) & Distance (m)"),
    subtitle = paste0(Match_Fixture)) %>%
  tab_source_note(md(My_caption)) %>%
  cols_label_with(fn = ~ janitor::make_clean_names(., case = "title")) %>%
...
Oppo_Passing_Chains_Table <- passing_sequencesOppo_Table %>%
  ...
  cols_align(align = "right", columns = everything()) %>%
  tab_header(
    title = paste0(Match_Oppo, " Passing Chain Length (5, 6-10, 11-15, 16 +) & Distance (m)"),
    subtitle = paste0(Match_Fixture)) %>%
  tab_source_note(md(My_caption)) %>%
  cols_label_with(fn = ~ janitor::make_clean_names(., case = "title")) %>%
  grand_summary_rows(columns = c(2:4, 6:8), fns = list("Overall Totals" = ~ sum (., na.rm=TRUE)), fmt = list(~fmt_number(., decimals =0))) %>%
