...
Vizlocation <- paste0("~/Dropbox/AI lfc data/UP Pod Prep 2324/Match day 27/")

#set dashboard directory
Dashlocation <- paste0("~/Dropbox/AI lfc data/UP Pod Prep 2324/Match day 27/Viz Pack/")

#set oppo
Match_Oppo <- paste("Nottingham Forest")

#set oppo color
Color_Oppo <- paste("#d3171e")
...
MatchIdUS <- 22162 #check

#Set Fotmob Match ID
MatchIdFotmob <- 4193798 #check
...
MatchIdFbref <- paste("https://fbref.com/en/matches/d1a00cf0/Nottingham-Forest-Liverpool-March-2-2024-Premier-League") #check

#set Game name
GameName <- paste("NFFC_vs_LFC")
...
GameNamePlayers <- paste("NFFC_vs_LFC_players")
...
ShotTitle_text_Away <- paste0(
  glue("<span style='font-family:\"Arial\";gray12;font-size:16px;'>Shot Map (</span><span style='font-family:\"Arial Bold\";color:{Away_Color};font-size:16px;'>Goal</span><span style='font-family:\"Arial\";color:gray12;font-size:16px;'> and </span><span style='font-family:\"Arial Bold\";color:#95A5A6;font-size:16px;'>No Goal</span><span style='color:gray12;font-size:16px;'>)</span>"))
...
lineupdataHome <- lineupdata %>%
  ...
  tab_style(
    style = list(cell_text(weight = "bold", color = Color_LFC_Text, align = "right"), cell_fill(color = Color_LFC)),
  ...
  tab_style(
    locations = cells_body(columns = vars(Sub_Off), rows = is.na(Sub_Off)),
    style = list(cell_text(color = "white"))) %>%
  tab_style(
    locations = cells_body(columns = vars(Sub_On), rows = is.na(Sub_On)),
    style = list(cell_text(color = "white"))) %>%
  tab_options(
...
``` {r MC Season total calculations, echo:FALSE, messages:FALSE, warnings:FALSE}
#load data
EPL2324ShotDatabase <- LFC_Understat_shots_EPL2324

#manipluate data
ShotsA <- EPL2324ShotDatabase %>%
  filter(home_away == "a")

ShotsH <- EPL2324ShotDatabase %>%
  filter(home_away == "h")

LFCShotsH <- ShotsH %>%
  filter(home_team == Match_LFC)

LFCShotsA <- ShotsA %>%
  filter(away_team == Match_LFC)
...
LFCShotsALL <- bind_rows(LFCShotsA, LFCShotsH)

OppoShotsALL <- bind_rows(OppoShotsA, OppoShotsH)

OppoShotsALL <- OppoShotsALL %>%
  select(xGA = xG)

LFCShotsALL <- LFCShotsALL %>%
  select(xG)

#predit seaosn on a match by match basis
# Set simulation parameters
n_sims <- 1000  # Number of simulations
n_games <- MatchDay    # Number of games in sample
remaining_games <- 38 - n_games
...
goals_scored <- rpois(n_sims * n_games, lambda = LFCShotsALL$xG)  # Poisson distribution for goals scored
goals_conceded <- rpois(n_sims * n_games, lambda = OppoShotsALL$xGA)  # Poisson distribution for goals conceded
...
season_totals <- sim_data %>%
  group_by(sim_id) %>%
  summarise(total_points = sum(points) + current_points)
...
```{r MC Season Plot}
#|height: 50%
# Create a histogram for the distribution of final points
Season_MC_Plot <- ggplot(point_probs, aes(x = total_points, y = prob)) +
  geom_bar(stat = "identity", fill = Color_LFC, color = "black") +
  labs(title = "Final Point Totals Post MD Result (1000 Sims)",
       subtitle = Match_Fixture,
       x = "Final Points",
       y = "Probability %",
       caption = my_socials) +
  scale_x_continuous(limits = c((min(season_totals$total_points)), (sum(current_points + remaining_games * 3))), breaks = seq(0, (sum(current_points + remaining_games * 3)), 1)) +
...
MatchSummarySlider <- plottingData

selectInput(
  "StartTime", "Select Start Time:",
  choices = unique(MatchSummarySlider$expandedMinute)
)
...
MatchSummarySliderData <- reactive({
  filtered_data <- MatchSummarySlider %>%
    filter(between(expandedMinute, input$StartTime, input$EndTime))
  return(filtered_data)
})

gt_output('MatchSummarySliderTable')
```


```{r slider summary table setup}
#|height: 25%

MatchSummarySliderTableData <- reactive({
  MatchSummarySliderData_value <- MatchSummarySliderData()  # Access the reactive value
  MatchSummarySliderTableData <- MatchSummarySliderData_value %>%
    mutate(Successful_Touch = ifelse(isTouch == "TRUE", 1, 0)) %>% 
    group_by(teamId) %>%  
    summarise_at(vars(Goals, In_play_time, Touches, PK_Box_Touches, Final_Third_Touches, Attacking_Half_Touches, Defensive_Half_Touches, TotalShots, OFShots, OTShots, SixYard_Box_Shots, PK_Box_Shots, Out_Box_Shots, xG, NPxG, xGOT, Attempted_Passes, Successful_Passes, Long_Passes, Successful_Long_Passes, BCs, Corners, Short_Corners), list(sum), na.rm = TRUE) %>%
    mutate(In_play_time = round((In_play_time/60),1),
  ...
  MatchSummarySliderTableData <- MatchSummarySliderTableData %>%
    left_join(PossessionTable, by = c("teamId" = "teamId"), copy = TRUE) %>%
    left_join(EPVTable, by = c("teamId" = "teamId"), copy = TRUE) %>%
    left_join(FieldTiltTable, by = c("teamId" = "teamId"), copy = TRUE) %>%
    mutate('EPV % Con.' = round((xG/EPV)*100,1))
  ...
  return(MatchSummarySliderTableData)
})

  MatchSummarySliderTable <- render_gt({
    MatchSummarySliderTableData_value <- MatchSummarySliderTableData() %>%
    gt(groupname_col = "Team") %>%
    cols_align(align = "right", columns = everything()) %>%
    tab_header(
      title = md("**Match Summary**"),
...
Oppotouchpass_bar_plot <- Oppotouchpass_bar_data %>%
  ...
  scale_x_continuous(breaks = seq(0, 100, 15)) +
  scale_y_continuous(limits = c(0, 30), breaks = seq(0, 30, 5)) +
  labs(title = OppoWavesTitle_text,
         subtitle = paste0(Match_Fixture),
         caption = My_caption,
         x = "Match Time (Mins)",
         y = "Metric Total") +
