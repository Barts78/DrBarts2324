...
Vizlocation <- paste0("~/Dropbox/AI lfc data/UP Pod Prep 2324/Match day 26/")

#set dashboard directory
Dashlocation <- paste0("~/Dropbox/AI lfc data/UP Pod Prep 2324/Match day 26/Viz Pack/")

#set oppo
Match_Oppo <- paste("Luton Town")

#set oppo color
Color_Oppo <- paste("#FA4616")

#set oppo text
Color_Oppo_Text <- paste("#27408B")

#Set LFC
Match_LFC <- paste("Liverpool")

#set Home color
Color_LFC <- paste("#d3171e")
...
ColorScale_Away <- paste("ggsci::orange_material") 

#set home color scale con
ColorScale_HomeC <- paste("ggthemes::Classic Red")

#set away color scale con
ColorScale_AwayC <- paste("ggthemes::Orange")

#set match date
Game_date <- paste("22-02-24")
...
Away_Color = Color_Oppo
Home_Color = Color_LFC
...
MatchIdUS <- 22151 #check

#Set Fotmob Match ID
MatchIdFotmob <- 4193787 #check
...
MatchIdFbref <- paste("https://fbref.com/en/matches/58636a1e/Liverpool-Luton-Town-February-21-2024-Premier-League") #check

#set Game name
GameName <- paste("LFC_vs_LTFC")

GameWeek <- paste("MD26")

#set Game name PLayers
GameNamePlayers <- paste("LFC_vs_LTFC_players")
...
DribblesTitle_text <- paste0(
  ...
  'and',
  '<span style="color:#0000FF"> **Successful**</span>','), ')
...
#    mutate(teamId = case_when(teamId == "Luton" ~ "Luton Town", TRUE ~ as.character(teamId)))
...
Oppo_shotsUS <- LFC_Understat_shots_EPL2324 %>%
    mutate(away_team = case_when(away_team == "Luton" ~ "Luton Town", TRUE ~ as.character(away_team)))
...
plot_passnet_AI <- function(data, team_name, scale_stat = "EPV", 
  ...
  '<span style="color:#0000FF"> **Successful**</span>','), ')
    ...
    plot_passnet <- ggplot() +
      ...
      scale_fill_manual(values = c("#0000FF",  "#BD1100"), labels = c("Won", "Lost"), name = "")
...
#LFCpassPlot1stWD_I <- ggplotly(LFCpassPlot1stWD, layerData = 12)
...
Player_Summary_plot_data <- MatchSummaryPLayerProfileData %>%
  select(Team = teamId, Name = playerId, role, '% Possession' = Possession, `% On Ball Time` = On_Ball_Time, Touches = Touches, 'Total Shots' = TotalShots, 'OFT Shots', 'OT Shots', 'Out Box Shots' = Out_Box_Shots, BCs, Goals, xG, xGOT, '% Team xG' = Team_xG_P, Touches, PK_Box_Touches, 'Attempted Passes' = Attempted_Passes, 'Successful Passes' = Successful_Passes, 'Pass Success %' = Pass_Success, Assists, 'Key Passes' = Key_Passes, EPV = TeamEPV, '% EPV' = Team_EPV_P, 'Aerials Total' = Aerial_total, 'Aerials Won' = Aerial_Won, 'Aerial Success %' = Aerial_Success, 'Duels Total' = Ground_Duels_Total, 'Duels Won' = Ground_Duels_Won, 'Duel Success %' = Duel_Success, '% Duels' = Team_Duels_P, '% Passes' = Team_Passes_P, '% Shots' = Team_Shots_P, '% Aerials' = Team_Aerial_P)
